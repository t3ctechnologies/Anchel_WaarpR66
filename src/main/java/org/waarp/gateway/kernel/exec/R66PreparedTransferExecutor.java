 ...
 * general to be placed in -info part (for instance ##UUID## giving #uuid#)<br>
 * <br>
 * So for instance "-to Host -file #BASEPATH##FILE# -rule RULE [-md5] [-nolog]
 * [-delay +delay] [-info ##UUID## #USER# #ACCOUNT# #COMMAND# INFO]" <br>
...
public class R66PreparedTransferExecutor extends AbstractExecutor {
	 ...
	 * Internal Logger
	 */
	private static final WaarpLogger logger = WaarpLoggerFactory.getLogger(R66PreparedTransferExecutor.class);
	...
	public R66PreparedTransferExecutor(String command, long delay, WaarpFuture futureCompletion) {
		...
		for (int i = 0; i < args.length; i++) {
			if (args[i].equalsIgnoreCase("-to")) {
				...
				i++;
				if (args[i].charAt(0) == '+') {
					timestart = new Timestamp(System.currentTimeMillis() + Long.parseLong(args[i].substring(1)));
				} else {
	...
	private void receiveRule() throws Reply421Exception {
		String message = "R66Prepared with -to " + remoteHost + " -rule " + rulename + " -file " + filename
				+ " -nolog: " + nolog + " -isMD5: " + isMD5 + " -info " + fileinfo;
		if (remoteHost == null || rulename == null || filename == null) {
			logger.error(
					"Mandatory argument is missing: -to " + remoteHost + " -rule " + rulename + " -file " + filename);
		...
		try {
			...
			throw new Reply421Exception("Cannot get Rule: " + rulename + "\n    " + message);
		}
		File tempFile = new File(filename);
		filename = tempFile.getName();
		String toFolderPath = rule.getRecvPath().substring(rule.getRecvPath().indexOf('/') + 1) + "/T0/"+filename;
		...
		if (!toFolderFile.exists()) {
			...
			RequestPacket request = new RequestPacket(rulename, mode, filename, blocksize, 0, DbConstant.ILLEGALVALUE,
					fileinfo, originalSize, sep);
			// Not isRecv since it is the requester, so send => isRetrieve is
			// true
			boolean isRetrieve = !RequestPacket.isRecvMode(request.getMode());
			logger.debug("Will prepare: {}", request);
			DbTaskRunner taskRunner;
			try {
				taskRunner = new DbTaskRunner(dbsession, rule, isRetrieve, request, remoteHost, timestart);
			} catch (WaarpDatabaseException e) {
	...
	private void sendRule() throws Reply421Exception {
		String message = "R66Prepared with -to " + remoteHost + " -rule " + rulename + " -file " + filename
				+ " -nolog: " + nolog + " -isMD5: " + isMD5 + " -info " + fileinfo;
		if (remoteHost == null || rulename == null || filename == null) {
			logger.error(

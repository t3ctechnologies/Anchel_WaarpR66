...
public class R66Session implements SessionInterface {
	 ...
	 * Internal Logger
	 */
	private static final WaarpLogger logger = WaarpLoggerFactory.getLogger(R66Session.class);
		...
		if (runner != null && (!runner.isFinished()) && (!runner.continueTransfer())) {
			if (localChannelReference != null) {
				if (!localChannelReference.getFutureRequest().isDone()) {
					R66Result result = new R66Result(new OpenR66RunnerErrorException("Close before ending"), this, true,
							ErrorCode.Disconnection, runner);// True since
																// called from
																// closed
	...
	public void partialClear() {
		...
		/*
		 * if (dir != null) { dir.clear(); } if (auth != null) { auth.clear(); }
	...
	public void setNoSessionRunner(DbTaskRunner runner, LocalChannelReference localChannelReference) {
		...
		if (this.localChannelReference == null) {
			...
			this.localChannelReference.setErrorMessage(this.runner.getErrorInfo().mesg, this.runner.getErrorInfo());
		}
	...
	public void setFileBeforePreRunner() throws OpenR66RunnerErrorException {
		...
		try {
			file = FileUtils.getFile(logger, this, this.runner.getOriginalFilename(), this.runner.isPreTaskStarting(),
					this.runner.isSender(), this.runner.isSendThrough(), file);
		} catch (OpenR66RunnerErrorException e) {
	 ...
	 * 
	 * @param createFile
	 *            When True, the file can be newly created if needed. If False,
	 *            no new file will be created, thus having an Exception.
	 * @throws OpenR66RunnerErrorException
	...
	public void setFileAfterPreRunner(boolean createFile) throws OpenR66RunnerErrorException, CommandAbstractException {
		...
		if (this.runner.isSender()) {
			...
			if (runner.getRank() > 0) {
				...
				try {
					file = (R66File) dir.setFile(this.runner.getFilename(), true);
				...
				if (createFile) {
					...
					try {
						file = dir.setUniqueFile(this.runner.getSpecialId(), newfilename);
	 ...
	 *            if True, the file as Sender should not be external to current
	 *            directory
	 * @throws OpenR66RunnerErrorException
	 */
			...
			if (!this.runner.isSender()) {
				...
				if (runner.isRecvThrough()) {
					...
					if (available > 0 && needed > available) {
						...
						throw new OpenR66RunnerErrorException(
								"File cannot be written due to unsufficient space available: " + targetDir + " need "
										+ needed + " more while available is " + available);
					}
					if (file == null) {
					...
					try {
						...
						if (oldPosition > length) {
							...
							logger.info("OldPos: " + oldPosition + ":" + runner.getRank() + " curLength: " + length
									+ ":" + newRank);
	...
	public void setFinalizeTransfer(boolean status, R66Result finalValue)
		...
		if (runner.isAllDone()) {
			...
			// FIXME ??
			/*
			 * if (! status) runner.finalizeTransfer(localChannelReference,
			 * file, finalValue, status);
			 */
			return;
		}
		if (localChannelReference.getFutureRequest().isDone()) {
		...
		if (!runner.ready()) {
			...
			if (!status && finalValue.getException() != null) {
				runnerErrorException = new OpenR66RunnerErrorException("Pre task in error (or even before)",
						finalValue.getException());
			} else {
				runnerErrorException = new OpenR66RunnerErrorException("Pre task in error (or even before)");
			}
			finalValue.setException(runnerErrorException);
			logger.debug("Pre task in error (or even before) : " + runnerErrorException.getMessage());
		...
		try {
			...
			if (status) {
				result = new R66Result(new OpenR66RunnerErrorException(e1), this, false, ErrorCode.Internal, runner);
			}
	...
	public void tryFinalizeRequest(R66Result errorValue)
		...
		if (runner.getStatus() == ErrorCode.CompleteOk) {
			...
			localChannelReference.validateRequest(new R66Result(this, true, ErrorCode.CompleteOk, runner));
		} else if (runner.getStatus() == ErrorCode.TransferOk
				&& ((!runner.isSender()) || errorValue.getCode() == ErrorCode.QueryAlreadyFinished)) {
			// Try to finalize it
			// status = true;
			try {
				this.setFinalizeTransfer(true, new R66Result(this, true, ErrorCode.CompleteOk, runner));
	...
	public String toString() {
		return "Session: FS[" + state.getCurrent() + "] " + status + "  " + (auth != null ? auth.toString() : "no Auth")

...
public class ServerInitDatabase {
	...
	protected static String _INFO_ARGS = Messages.getString("ServerInitDatabase.Help"); //$NON-NLS-1$
	...
	public static void initR66database(String[] args) {
		...
		try {
			if (!FileBasedConfiguration.setConfigurationInitDatabase(Configuration.configuration, args[0])) {
			...
			if (slimitconfig != null) {
				// Load configuration
				if (args.length > 2) {
					FileBasedConfiguration.setConfigurationLoadLimitFromXml(Configuration.configuration, slimitconfig);
				}
			}
			...
			if (sbusiness != null || salias != null || sroles != null) {
				...
				try {
					...
					hostConfiguration = new DbHostConfiguration(DbConstant.admin.getSession(),
							Configuration.configuration.getHOST_ID(), sbusiness, sroles, salias, "");
					try {
	...
	public static boolean upgradedb() {
		...
		String version = DbHostConfiguration.getVersionDb(DbConstant.admin.getSession(),
				Configuration.configuration.getHOST_ID());
		try {
			if (version != null) {
	...
	public static void loadHostAuth(String filename) {
		AuthenticationFileBasedConfiguration.loadAuthentication(Configuration.configuration, filename);
	}
}
